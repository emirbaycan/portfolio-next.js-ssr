...
export async function generateMetadata({
  ...
  return {
    title: `Career Detail - ${params.careerId}`,
    description: `Details for career entry ${params.careerId}`,
  };
}
...
export default function CareerDetailPage(props: any) {
  const { params } = props;
  // @ts-ignore
        ...
        <div className="mb-4 flex justify-center sm:justify-start">
          <div className="inline-flex items-center bg-background border border-primary px-2 py-0.5 rounded-full text-xs font-medium text-primary shadow-sm">
            {typeof career.endDate === "string"
              ? getYearFromDate(career.startDate) + " - " + t("present")
